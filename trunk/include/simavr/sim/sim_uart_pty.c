		...
		uart_pty_t * p)
{
	while (p->xon && !uart_pty_fifo_isempty(&p->pty.out)) {
		...
		printf("uart_pty_flush_incoming send r %03d:%02x\n", r, byte);
...
{
	uart_pty_t * p = (uart_pty_t*)param;
	if (!p->xon) printf("uart_pty_xon_hook\n");
	p->xon = 1;
...
{
	uart_pty_t * p = (uart_pty_t*)param;
	if (p->xon) printf("uart_pty_xoff_hook\n");
	p->xon = 0;
